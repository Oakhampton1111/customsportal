...
async def get_document_statistics(
    ...
    try:
        ...
        total_docs_result = await db.execute(
            db.query(func.count(Document.id), func.sum(Document.file_size))
        ...
        compliance_stats = await db.execute(
            db.query(Document.compliance_status, func.count(Document.id))
            .filter(Document.is_active == True)
        ...
        pending_review = await db.scalar(
            ...
            .filter(and_(Document.is_active == True, Document.status == DocumentStatus.PENDING_REVIEW))
        )
        ...
        compliance_issues = await db.scalar(
            ...
            .filter(and_(
                Document.is_active == True,
                Document.compliance_status.in_([ComplianceStatus.NON_COMPLIANT, ComplianceStatus.NEEDS_REVIEW])
            ))
        )
        ...
        recent_uploads = await db.scalar(
            db.query(func.count(Document.id))
            .filter(and_(Document.is_active == True, Document.upload_date >= week_ago))
        )
        
        # Top uploaders
        uploader_stats = await db.execute(
            db.query(Document.uploaded_by, func.count(Document.id))
            .filter(Document.is_active == True)
        ...
        storage_stats = await db.execute(
            db.query(Document.document_type, func.sum(Document.file_size))
            .filter(Document.is_active == True)
            .group_by(Document.document_type)
        )
...
async def list_documents(
    ...
    try:
        ...
        total_count = await db.scalar(
            query.with_only_columns(func.count(Document.id))
        )
...
async def search_documents(
    ...
    try:
        ...
        if search_params.category:
            query = query.filter(Document.category == search_params.category)
        if search_params.status:
            query = query.filter(Document.status == search_params.status)
        if search_params.compliance_status:
            query = query.filter(Document.compliance_status == search_params.compliance_status)
        if search_params.client_id:
            query = query.filter(Document.client_id == search_params.client_id)
        if search_params.client_name:
            query = query.filter(Document.client_name.ilike(f"%{search_params.client_name}%"))
        if search_params.hs_code:
            query = query.filter(Document.hs_code == search_params.hs_code)
        if search_params.shipment_ref:
            query = query.filter(Document.shipment_ref == search_params.shipment_ref)
        if search_params.is_confidential is not None:
            query = query.filter(Document.is_confidential == search_params.is_confidential)
        if search_params.uploaded_by:
            query = query.filter(Document.uploaded_by == search_params.uploaded_by)
        if search_params.upload_date_from:
            query = query.filter(Document.upload_date >= search_params.upload_date_from)
        if search_params.upload_date_to:
            query = query.filter(Document.upload_date <= search_params.upload_date_to)
